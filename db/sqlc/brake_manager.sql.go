...
const getAllBrakeManager = `-- name: GetAllBrakeManager :many
SELECT id, created_at, state, hydrolic_pressure_loss, critical_pod_acceleration_mesure_timeout, critical_pod_deceleration_instruction_timeout, verin_blocked, emergency_valve_open_without_hydrolic_pressor_diminution, critical_emergency_brakes_without_deceleration, mesured_distance_less_than_desired, mesured_distance_greater_as_desired
FROM brake_managers
ORDER BY id
`

func (q *Queries) GetAllBrakeManager(ctx context.Context) ([]BrakeManager, error) {
	rows, err := q.db.QueryContext(ctx, getAllBrakeManager)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getLatestBrakeManager = `-- name: GetLatestBrakeManager :one
SELECT id, created_at, state, hydrolic_pressure_loss, critical_pod_acceleration_mesure_timeout, critical_pod_deceleration_instruction_timeout, verin_blocked, emergency_valve_open_without_hydrolic_pressor_diminution, critical_emergency_brakes_without_deceleration, mesured_distance_less_than_desired, mesured_distance_greater_as_desired
FROM brake_managers
ORDER BY id DESC
LIMIT 1
...
func (q *Queries) GetLatestBrakeManager(ctx context.Context) (BrakeManager, error) {
	row := q.db.QueryRowContext(ctx, getLatestBrakeManager)
	var i BrakeManager
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
